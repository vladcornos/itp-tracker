...
export default function App() {
  ...
  async function addVehicle() {
    ...
    if (!error) {
      ...
      setForm({ owner_name: "", plate: "", phone: "", itp_date: "" });
      setShowForm(false);
      fetchVehicles();
        ...
        <div style={{ maxWidth: 800, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div>
              ...
              <div style={{ color: "rgba(255,255,255,0.5)", fontSize: 11 }}>ITP expiră după 12 luni</div>
            </div>
          </div>
          <button onClick={() => { setShowForm(!showForm); setMsg(""); }}
            style={{ background: showForm ? "rgba(255,255,255,0.1)" : "#e63946", color: "#fff", border: "none", borderRadius: 10, padding: "8px 16px", fontSize: 13, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}>
      ...
      <div style={{ maxWidth: 800, margin: "0 auto", padding: "1.25rem" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 10, marginBottom: 20 }}>
          {[
            ...
            <div key={label} style={{ background: bg, border: `1px solid ${color}30`, borderRadius: 14, padding: "1rem", backdropFilter: "blur(10px)" }}>
        ...
        {showForm && (
          <div style={{ background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 16, padding: "1.25rem", marginBottom: 20, animation: "fadeIn 0.3s ease" }}>
            ...
            <button onClick={addVehicle}
              style={{ marginTop: 12, width: "100%", height: 44, background: "#e63946", color: "#fff", border: "none", borderRadius: 10, fontSize: 15, fontWeight: 600, cursor: "pointer" }}>
              ✅ Salvează vehicul
            </button>
        ...
        {loading ? (
          ...
          const percent = Math.max(0, Math.min(100, Math.round((days / 365) * 100)));
            ...
            <div key={v.id} className="card" style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 16, padding: "1.1rem", marginBottom: 12, transition: "transform 0.2s" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                <div>
                  ...
                  <div style={{ color: "rgba(255,255,255,0.5)", fontFamily: "monospace", fontSize: 13, marginTop: 2 }}>{v.plate}</div>
                </div>
